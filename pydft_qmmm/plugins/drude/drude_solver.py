...
class DrudeSCFIterator:
    ...
    def is_converged(self):
        last_forces = None
        second_last_forces = None
        if len(self.history["drude_forces"]) >= 1:
            last_forces = self.history["drude_forces"][-1]
            rms_force = np.sqrt(np.mean(last_forces**2))
            if rms_force <= self.force_tolerance:
                return True
            
        if len(self.history["drude_forces"]) >= 2:
            force_squared = np.sum(last_forces**2)
            second_last_forces = self.history["drude_forces"][-2]
            second_last_forces_squared = float(np.sum(second_last_forces**2))
        if (
                self.stopping_ratio is not None
                and force_squared > self.stopping_ratio*second_last_forces_squared
            ):
            return True
        return False
